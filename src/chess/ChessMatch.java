...
public class ChessMatch {
	...
	public ChessPiece replacePromotedPiece(String type) {
		...
		if(!(type.equals("B")||type.equals("Q")||type.equals("N"))) {
			throw new IllegalStateException("Invalid type promotion");
		}
	...
	private void validateSourcePosition(Position position) {
		...
		if (!board.piece(position).isThereAnyPossibleMove()) {
			throw new ChessException("There is no moves possible for the chosen piece");
		}
	...
	private boolean testCheck(Color color) {
		Position kingProsition = king(color).getChessPosition().toPosition();
		List<Piece> opponentPieces = piecesOnTheBoard.stream()
				.filter(x -> ((ChessPiece) x).getColor() == opponent(color)).collect(Collectors.toList());
		for (Piece p : opponentPieces) {
			boolean[][] mat = p.possibleMoves();
			if (mat[kingProsition.getRow()][kingProsition.getColumn()]) {
				return true;
			}
		}
		return false;
